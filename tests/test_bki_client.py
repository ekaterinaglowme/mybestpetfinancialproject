"""Клиент БКИ: ретрай с паузой, сырой ответ при сбое разбора.

Клиент лишь возвращает status="unavailable" при сбое; отказывать по нему
(fail-closed) — дело вызывающего кода, это проверяется в тестах решения/ручки.
"""
